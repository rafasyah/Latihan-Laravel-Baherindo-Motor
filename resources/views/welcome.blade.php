...
@section('content')
 ...
 <h1 class="text-[40px] text-center font-bold p-5">Welcome to Baherindo Motor</h1>
    <p class="text-center">Jual Beli Motor second termurah di Bekasi</p>
...
<div class="page-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
    ...
    <div class="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
        <img class="w-full h-48 object-cover" src="{{ asset($m['img']) }}" alt="{{ $m['name'] }}">

        <div class="p-6">
            <h2 class="text-xl font-semibold text-gray-800 truncate">{{ $m['name'] }}</h2>
            <div class="mt-4 flex items-center justify-between">
                <span class="text-blue-600 font-bold text-lg">Rp {{ number_format($m['price'], 0, ',', '.') }}</span>
